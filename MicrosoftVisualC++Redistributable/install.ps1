...
Write-Host "`r`n### $title 시작 ###`r`n"

$OSBit = if ([IntPtr]::Size -eq 4) {32} else {64}
$OSArchs = 86, 64
$restart = $null

$data = Invoke-RestMethod 'https://raw.githubusercontent.com/ssokka/Windows/master/MicrosoftVisualC++Redistributable/install.json'
[array]::Reverse($data)
...
foreach ($item in $data) {
	...
	foreach ($OSArch in $OSArchs) {
		...
		$name = '{0}{1} x{2}' -f $item.Product, $ServicePackName, $OSArch
		$file = '{0}\mvcr-{1}{2}-x{3}.exe' -f $env:TEMP, $item.Product, $ServicePackFile, $OSArch
		$log = '{0} "{1}"' -f $item.LogOption, $file -replace '\.exe', '.log'
		if ($item.Product -eq 2005) {$log = ''}
		$CLO = '{0} {1}' -f $item.CommandLineOptions, $log
		$url = $item.'x{0}' -f $OSArch
		$url
		continue
		...
		if ($process.ExitCode -ne 0 -and $process.ExitCode -ne 3010) {Write-Host " 실패 !" -ForegroundColor Red}
	}
	Write-Host "`r`n" 
}
...
if ($restart) {
	Write-Host "설치를 완료하려면 컴퓨터를 다시 시작해야 합니다.`r`n지금 컴퓨터를 다시 시작하시겠습니까? [Y/N]" -NoNewline -ForegroundColor Red
	$input = Read-Host
